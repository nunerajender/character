...
class Character::ApiController < Character::BaseController
  ...
  def set_form_template
    ...
    if template_folder.start_with? 'character/'
      template_folder = 'character/' + template_folder.gsub('character/', '').gsub('/', "/#{ current_namespace.name }/")
    else
      template_folder = "character/#{ current_namespace.name }/" + template_folder
    end
  ...
  def new
    @form_action_url = "/#{ current_namespace.name }/#{ @model_slug }"
  ...
  def edit
    @object = @model_class.find(params[:id])
    @form_action_url = "/#{ current_namespace.name }/#{ @model_slug }/#{ @object.id }"
    render @form_template
  end
  ...
  def check_permissions
    filter = current_namespace.request_filter
    if filter.nil? || self.instance_exec(&filter)
      true
    else
      redirect_to "/#{ current_namespace.name }", warning: "Action not permitted"
    end
  end
